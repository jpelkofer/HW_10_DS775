...
#b)
two_mill <- pnorm(2000000, mean = avg_NPV, sd = sd_NPV, lower.tail = FALSE)
two_mill

#c)
min_profits <- c(min(Yr2_cash_flow), min(Yr3_cash_flow), min(Yr4_cash_flow), min(Yr5_prof_or_loss))

#d)
#Probability of annual operating profit of at least 0 in all four years
zero_profit <- pnorm(0, mean = mean(Yr4_cash_flow), sd = sd(Yr4_cash_flow), lower.tail = FALSE)^4
zero_profit
```